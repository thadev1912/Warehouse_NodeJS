...
let index = async (req, res) => {
    try {
        let phieuxuatkho = await db.Phieuxuatkho.findAll();
        if (phieuxuatkho) {
            res.status(200).json({
                success: true,
                data: phieuxuatkho,
...
let store = async (req, res) => {
    try {
        ...
        let vattu = await db.Phieuxuatkho.create({
            ma_phieu: req.body.ma_phieu,
            id_nhanvien: req.body.id_nhanvien,
            id_thukho: req.body.id_thukho,
...
let edit = async (req, res) => {
    try {
        //let ls_lop=await db.Phieuxuatkho.findAll();
        let id = req.params.id;
        let vattu = await db.Phieuxuatkho.findOne({
            where: { id: id },
            raw: true,
        });
        if (vattu) {
...
let update = async (req, res) => {
    try {
        ...
        let id = req.params.id;
        console.log(id);
        exitsId = await db.Phieuxuatkho.findOne({
            where: { id: id },
        });
        if (exitsId) {
            let vattu = await db.Phieuxuatkho.update({
                ma_phieu: req.body.ma_phieu,
                id_nhanvien: req.body.id_nhanvien,
                id_thukho: req.body.id_thukho,
                id_nhacc: req.body.id_nhacc,
...
let destroy = async (req, res) => {
    try {
        ...
        if (exitsId) {
            let vattu = await db.Phieuxuatkho.destroy({
                where: { id: id },
            });
            res.status(200).json({
...
let search = async (req, res) => {
    try {
        ...
        let searchphieuxuatkho = await db.Phieuxuatkho.findAll({
            where: {
                ma_phieu: { [Op.like]: '%' + txtSearch + '%' },
            }
        });
        if (searchphieuxuatkho.length === 0) {
            res.status(404).json({ message: 'Không tìm thấy dữ liệu nào cả!!!' });
        }
        else {
            res.status(200).json({ success: true, data: searchphieuxuatkho, message: 'Search data comepleted' });
        }
