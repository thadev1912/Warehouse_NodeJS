...
let index = async (req, res) => {
    try {
        let lydoxuat = await db.LydoXuat.findAll();
        if (lydoxuat) {
            res.status(200).json({
                success: true,
                data: lydoxuat,
...
let store = async (req, res) => {
    try {
        ...
        let vattu = await db.LydoXuat.create({
            lydo: req.body.lydo,
...
let edit = async (req, res) => {
    try {
        //let ls_lop=await db.LydoXuat.findAll();
        let id = req.params.id;
        let vattu = await db.LydoXuat.findOne({
            where: { id: id },
            raw: true,
        });
        if (vattu) {
...
let update = async (req, res) => {
    try {
        ...
        let id = req.params.id;
        console.log(id);
        exitsId = await db.LydoXuat.findOne({
            where: { id: id },
        });
        if (exitsId) {
            let vattu = await db.LydoXuat.update({
                lydo: req.body.lydo,
            }, { where: { id: id } });
            if (vattu) {
                res.json({
...
let destroy = async (req, res) => {
    try {
        ...
        if (exitsId) {
            let vattu = await db.LydoXuat.destroy({
                where: { id: id },
            });
            res.status(200).json({
...
let search = async (req, res) => {
    try {
        ...
        let searchvattu = await db.LydoXuat.findAll({
            where: {
